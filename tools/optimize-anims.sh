#!/usr/bin/env bash
# Uso: bash tools/optimize-anims.sh <entrada.glb> <nombre>
# Deja solo las animaciones de un GLB exportado de Tripo (tools/strip-anim.mjs) y las
# comprime en assets/models/<nombre>.glb, el mismo fichero para móvil y ordenador.
set -euo pipefail
input="$1"; name="$2"
cd "$(dirname "$0")/.."
...
t="$tmp/$name"
node tools/strip-anim.mjs "$input" "$t-1.glb"
